...
*/
const filterProduct = products.filter(product => product.inStock);
...
*/
//let discountProduct = products.map(product => )
...
*/ 
let applyDiscount => (discountPercent){
  return function(product){
    let discount = product.price * (discountPercent / 100);
    return {
      ... product,
...
let discountedProducts = products.map(product => product * (1 - (discountedPercentage / 100)));
...
let totalInventoryValue = products.reduce((acc, product) => {
